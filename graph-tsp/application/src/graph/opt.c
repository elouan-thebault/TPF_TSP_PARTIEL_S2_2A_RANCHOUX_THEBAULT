...
void two_opt(Path* path, Graph* graph, int** cl, int max_passes)
{
    ...
    for (ListIntNode* it = s->next; it != s; it = it->next) tour[count++] = it->value;
    ...
    int idx = 0;
    for (ListIntNode* it = s->next; it != s; it = it->next) it->value = tour[idx++];
        ...
        for (int seg_len = 1; seg_len <= 2; seg_len++)
        {
            for (int i = 1; i + seg_len <= n; i++)
            {
                ...
                if (best_j >= 0)
                {
                    ...
                    if (best_j < i)
                    {
                        for (int k = i - 1; k > best_j; k--) tour[k + seg_len] = tour[k];
                        for (int k = 0; k < seg_len; k++) tour[best_j + 1 + k] = seg[k];
                    }
                    else
                    {
                        for (int k = i + seg_len; k <= best_j; k++) tour[k - seg_len] = tour[k];
                        for (int k = 0; k < seg_len; k++) tour[best_j - seg_len + 1 + k] = seg[k];
                    }
    ...
    float q)
{
    ...
    int cl_size = (n - 1 < NBR_MAX_VOISIN) ? (n - 1) : NBR_MAX_VOISIN;
    int** candidates = build_candidate_lists(distances, cl_size);

    Path* bestPath = Graph_tspFromHeuristic(distances, station);
    if (bestPath)
    {
        two_opt(bestPath, distances, candidates, 5);
        or_opt(bestPath, distances, candidates);
        two_opt(bestPath, distances, candidates, 5);
    ...
    for (int iter = 0; iter < iterationCount; iter++)
    {
        ...
        {
            Path* p = aco_constructPathCL(distances, pheromones,station, alpha, beta,candidates);

            if (!p) continue;

            two_opt(p, distances, candidates, 1);
        ...
        or_opt(iterationBest, distances, candidates);
        two_opt(iterationBest, distances, candidates, 5);

        Graph_acoPheromoneUpdatePath(pheromones, iterationBest, q);

        if (bestPath) Graph_acoPheromoneUpdatePath(pheromones, bestPath, q * 5.0f);

        if (!bestPath || iterationBest->distance < bestPath->distance)
        {
